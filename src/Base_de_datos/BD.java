...
public class BD {
	...
	private static final String CREATE_ENUM_TIPO_SERVICIO = 
			"DROP TYPE IF EXISTS TipoServicio CASCADE;"
			+ "CREATE TYPE TipoServicio AS ENUM('Wifi', 'AirConditioner');";
	
	private static final String ELIMINAR_SCHEMA = 
			"DROP SCHEMA public CASCADE;"
			+ "CREATE SCHEMA public;"
			+ "SET search_path TO public;";
	
	private static final String CREATE_TABLA_PARADA = 
			"CREATE TABLE IF NOT EXISTS parada ("
			+ "nroParada INT PRIMARY KEY,"
			+ "numeroCalle INT,"
			+ "nombreCalle VARCHAR(40),"
			+ "estado BOOLEAN"
			+ ");";
	
	private static final String CREATE_TABLA_BOLETO =
			"CREATE TABLE IF NOT EXISTS boleto ("
			+ "id INT PRIMARY KEY,"
			+ "MONTO_POR_KM FLOAT NOT NULL,"
			+ "precio FLOAT NOT NULL,"
			+ "fechaViaje DATE,"
			+ "id_bus INT REFERENCES Bus(numero),"
			+ "id_linea INT REFERENCES Linea(id)"
			+ ");";
	
	private static final String CREATE_TABLA_BUS = 
			"CREATE TABLE IF NOT EXISTS bus ("
			+ "numero INT PRIMARY KEY,"
			+ "id_linea INT REFERENCES Linea(id),"
			+ "parada_inicio INT NOT NULL REFERENCES Parada(nroParada),"
			+ "parada_fin INT NOT NULL REFERENCES Parada(nroParada),"
			+ "parada_auxiliar INT REFERENCES Parada(nroParada)"
			+ ");";
	
	private static final String CREATE_TABLA_LINEA = 
	...
	private static final String CREATE_TABLA_LINEA_ECONOMICA = 
			"CREATE TABLE IF NOT EXISTS linea_economica ("
			+ "id INT REFERENCES Linea(id),"
			+ "porcentaje FLOAT CHECK (porcentaje > 0 AND porcentaje <= 4.0),"
			+ "COSTO_ECONOM FLOAT,"
			+ "PRIMARY KEY (id)"
			+ ");";
	
	private static final String CREATE_TABLA_LINEA_SUPERIOR = 
			"CREATE TABLE IF NOT EXISTS linea_superior ("
			+ "id INT REFERENCES Linea(id),"
	...
	private static final String CREATE_TABLA_CALLE = 
			"CREATE TABLE IF NOT EXISTS calle ("
			+ "id INT PRIMARY KEY,"
			+ "parada_inicial INT REFERENCES Parada(nroParada),"
			+ "parada_final INT REFERENCES Parada(nroParada),"
			+ "longitud FLOAT NOT NULL"
			+ ");";
	
	private static final String CREATE_TABLA_INCIDENTE = 
			"CREATE TABLE IF NOT EXISTS incidente ("
			+ "id INT PRIMARY KEY,"
			+ "nroParada INT REFERENCES Parada(nroParada),"
	...
	private static final String CREATE_TABLA_RUTA = 
	...
	private static final String CREATE_TABLA_PARADA_RUTA = 
			"CREATE TABLE IF NOT EXISTS parada_ruta ("
			+ "id_parada INT REFERENCES Parada(nroParada),"
	...
	private static final String INSERT_TABLA_PARADA = 
	...
	private static final String INSERT_TABLA_CALLE =
	...
	private static final String INSERT_TABLA_BOLETO = 
			"INSERT INTO boleto VALUES"
			+ "(1, 20, 200, '01/02/2022', 1, 1),"
			+ "(2, 22, 300, '17/03/2022', 2, 1),"
			+ "(3, 24, 400, '21/04/2022', 3, 2)"
			+ ";";
	
	private static final String INSERT_TABLA_BUS =
			"INSERT INTO bus VALUES"
			+ "(1, 1, 2, 8, null),"
			+ "(2, 1, 2, 8, null),"
			+ "(3, 2, 1, 4, null)"
			+ ";";
	private static final String INSERT_TABLA_INCIDENTE =
			"INSERT INTO incidente VALUES"
			+ "(1, 3, '01/02/2022', null, '', true),"
			+ "(2, 8, '15/03/2022', '19/03/2022', '', true),"
			+ "(3, 6, '01/01/2022', '10/01/2022', '', false)"
			+ ";";
	private static final String INSERT_TABLA_LINEA =
			"INSERT INTO linea VALUES"
			+ "(1, 'Azul', 'Linea1', 30, 20),"
			+ "(2, 'Verde', 'Linea2', 40, 25)"
			+ ";";
			
	private static final String INSERT_TABLA_LINEA_ECONOMICA =	
			"INSERT INTO linea_economica VALUES"
			+ "(1, 2.5, 0.02)"
			+ ";";
				
	private static final String INSERT_TABLA_LINEA_SUPERIOR =
			"INSERT INTO linea_superior VALUES"
			+ "(1, 'Wifi', 0.5, 0.5, 0.1)"
			+ ";";
	
	private static final String INSERT_TABLA_RUTA =
			"INSERT INTO ruta VALUES"
			+ "(1, 5, 3, 6, 4, 20, 15)"
			+ ";";
	
	private static final String INSERT_TABLA_PARADAS_RUTA = 
			"INSERT INTO parada_ruta VALUES"
			+ "(3, 1),"
			+ "(4, 1),"
			+ "(5, 1),"
			+ "(6, 1)"
...
private static void eliminar() {
	...
	private static void eliminarSchema(Connection conn) {
